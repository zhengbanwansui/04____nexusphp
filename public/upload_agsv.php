...
?>
<?php
$countUserTorrents = \Nexus\Database\NexusDB::table('torrents')->where('owner', $CURUSER["id"])->count();
if ($countUserTorrents > 3) {
    print('<form id="compose" enctype="multipart/form-data" action="takeupload.php" method="post" name="upload">');
} else {
    print('<form id="compose" enctype="multipart/form-data" action="takeupload_tmp.php" method="post" name="upload">');
}
?>
			<?php
...
JS;
				...
				?>
				<tr><td class="toolbox" align="center" colspan="2"><b><?php echo $lang_upload['text_read_rules']?></b> <input id="qr" type="submit" class="btn" value="<?php
                        if ($countUserTorrents > 3) {
                            echo $lang_upload['submit_upload'];
                        } else {
                            echo $lang_upload['submit_upload']."(当前发种数量不足三个，将自动发布到候选区)";
                        }
                        ?>" /></td></tr>
		</table>
	</form>
