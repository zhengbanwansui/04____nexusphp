...
dbconn();
require_once('lang/chs/lang_takeupload.php');
...
$id = \Nexus\Database\NexusDB::insert('torrents_tmp', $insert);
...
{$lang_takeupload_target[$langfolder_array[$i]]['mail_torrent']}<b><a href="javascript:void(null)" onclick="window.open('http://$BASEURL/details_tmp.php?id=$id&hit=1')">{$lang_takeupload_target[$langfolder_array[$i]]['mail_here']}</a></b><br />
http://$BASEURL/details_tmp.php?id=$id&hit=1
...
header("Location: " . get_protocol_prefix() . "$BASEURL/details_tmp.php?id=".htmlspecialchars($id)."&uploaded=1");
?>
