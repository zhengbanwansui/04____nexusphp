...
//require_once("lang/chs/lang_details.php");
//############agsv候选################@
loggedinorreturn();
$id = intval($_GET["id"] ?? 0);
...
$res = sql_query("SELECT torrents_tmp.cache_stamp, torrents_tmp.sp_state, torrents_tmp.url, torrents_tmp.small_descr, torrents_tmp.seeders, torrents_tmp.banned, torrents_tmp.leechers, torrents_tmp.info_hash, torrents_tmp.filename, nfo, LENGTH(torrents_tmp.nfo) AS nfosz, torrents_tmp.last_action, torrents_tmp.name, torrents_tmp.owner, torrents_tmp.save_as, torrents_tmp.descr, torrents_tmp.visible, torrents_tmp.size, torrents_tmp.added, torrents_tmp.views, torrents_tmp.hits, torrents_tmp.times_completed, torrents_tmp.id, torrents_tmp.type, torrents_tmp.numfiles, torrents_tmp.anonymous, torrents_tmp.pt_gen, torrents_tmp.technical_info, torrents_tmp.hr, torrents_tmp.promotion_until, torrents_tmp.promotion_time_type, torrents_tmp.approval_status, torrents_tmp.price,
       categories.name AS cat_name, categories.mode as search_box_id, $taxonomyFields
FROM torrents_tmp LEFT JOIN categories ON torrents_tmp.category = categories.id
    LEFT JOIN sources ON torrents_tmp.source = sources.id
    LEFT JOIN media ON torrents_tmp.medium = media.id
    LEFT JOIN codecs ON torrents_tmp.codec = codecs.id
    LEFT JOIN standards ON torrents_tmp.standard = standards.id
    LEFT JOIN processings ON torrents_tmp.processing = processings.id
    LEFT JOIN teams ON torrents_tmp.team = teams.id
    LEFT JOIN audiocodecs ON torrents_tmp.audiocodec = audiocodecs.id
WHERE torrents_tmp.id = $id LIMIT 1")
or sqlerr();
...
if (!$row) {
	...
	if (!isset($_GET["cmtpage"])) {
		...
		if (!empty($_GET["uploaded"]))
		{
			...
			header("refresh: 1; url=download_tmp.php?id=$id");
		...
		if ($CURUSER["downloadpos"] != "no")
		{
			...
			print("<a class=\"index\" href=\"download_tmp.php?id=$id\">" . htmlspecialchars($torrentnameprefix ."." .$row["save_as"]) . ".torrent</a>&nbsp;&nbsp;<a id=\"bookmark0\" href=\"javascript: bookmark(".$row['id'].",0);\">".get_torrent_bookmark_state($CURUSER['id'], $row['id'], false)."</a>&nbsp;&nbsp;&nbsp;".$lang_details['row_upped_by']."&nbsp;".$uprow.$uploadtime);
			print("</td></tr>");
		}
		else
...
//		tr($lang_details['row_basic_info'], $size_info.$type_info.$source_info . $medium_info. $codec_info . $audiocodec_info. $standard_info . $processing_info . $team_info, 1);
        ...
        if ($CURUSER["downloadpos"] != "no") {
            ...
            $actions[] = "<a title=\"".$lang_details['title_download_torrent']."\" href=\"download_tmp.php?id=".$id."\"><img class=\"dt_download\" src=\"pic/trans.gif\" alt=\"download\" />&nbsp;<b><font class=\"small\">".$downloadBtn."</font></b></a>";
        }
        if ($owned == 1) {
...
JS;
        ...
        tr($lang_details['torrent_dl_url'],sprintf('<a title="%s" href="%s/download_tmp.php?downhash=%s|%s">%s</a>',$lang_details['torrent_dl_url_notice'], getSchemeAndHttpHost(), $CURUSER['id'], $torrentRep->encryptDownHash($row['id'], $CURUSER), $lang_details['torrent_dl_url_text']),1);
...
//            $subTorrentIdArr = array_merge($subTorrentIdArr, $otherCopiesIdArr);
	...
	if (get_setting('main.enable_pt_gen_system') == 'yes' && !empty($row['pt_gen'])) {
		...
		{
//			$where_area = " url = " . sqlesc((int)$imdb_id) ." AND torrents_tmp.id != ".sqlesc($id);
			$where_area = sprintf('torrents_tmp.id in (%s)', implode(',', $otherCopiesIdArr));
			$copies_res = sql_query("SELECT torrents_tmp.id, torrents_tmp.name, torrents_tmp.sp_state, torrents_tmp.size, torrents_tmp.added, torrents_tmp.seeders, torrents_tmp.leechers, torrents_tmp.hr,categories.id AS catid, categories.name AS catname, categories.image AS catimage, $taxonomyFields, categories.mode as search_box_id FROM torrents_tmp
    LEFT JOIN categories ON torrents_tmp.category=categories.id
    LEFT JOIN sources ON torrents_tmp.source = sources.id
    LEFT JOIN media ON torrents_tmp.medium = media.id
    LEFT JOIN codecs ON torrents_tmp.codec = codecs.id
    LEFT JOIN standards ON torrents_tmp.standard = standards.id
    ...
    LEFT JOIN audiocodecs ON torrents_tmp.audiocodec = audiocodecs.id
    LEFT JOIN processings ON torrents_tmp.processing = processings.id
WHERE " . $where_area . " ORDER BY torrents_tmp.id DESC") or sqlerr(__FILE__, __LINE__);
...
//						$other_processing_info = $copy_row['processing_name'].", ";
					...
					$s .= "<tr". $sphighlight."><td class=\"rowfollow nowrap\" valign=\"middle\" style='padding: 0px'>".return_category_image($copy_row["catid"], "torrents_tmp.php?allsec=1&amp;")."</td><td class=\"rowfollow\" align=\"left\"><a href=\"" . htmlspecialchars(get_protocol_prefix() . $BASEURL . "/details.php?id=" . $copy_row["id"]. "&hit=1")."\">" . $dispname ."</a>". $sp_info. $hrImg ."</td>" .
