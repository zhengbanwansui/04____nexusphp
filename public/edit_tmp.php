...
$res = sql_query("SELECT torrents_tmp.*, categories.mode as cat_mode FROM torrents_tmp LEFT JOIN categories ON category = categories.id WHERE torrents_tmp.id = $id");
...
if (!isset($CURUSER) || ($CURUSER["id"] != $row["owner"] && !user_can('torrentmanage'))) {
	...
	print("<form method=\"post\" id=\"compose\" name=\"edittorrent\" action=\"takeedit_tmp.php\" enctype=\"multipart/form-data\">");
